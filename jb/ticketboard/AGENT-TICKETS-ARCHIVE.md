...
**Review:** Approve (2026-08-11) — thread-safety verified by reading the actual source, not just the
commit's claims: all 12 `Analyzer_*` types are stateless `static` classes (confirmed), `FeatureAnalysisService`
and `SubjectDetector` hold only `readonly` fields set once at construction with no shared mutable state
across calls (confirmed, `SubjectDetector.cs` full read), `YoloDetector.Detect` serializes on its own
`RunLock` (confirmed at `YoloDetector.cs:90`), and `familyById` is built to completion before
`Parallel.ForEach` starts (confirmed in the diff itself). Independently re-ran everything rather than
trusting the ticket's numbers: `RefinePhenotypesParallelTests` 2/2 pass (40 real CiMini images, sized
so `Parallel.ForEach` genuinely engages multiple threads — not a token 2-3 image check — asserting both
no dropped `Interlocked` counter and identical phenotypes across two full runs); full
`Prism.Services.Matching.Tests` 276/276 (ticket claimed 275/275 — one more now, still all green, no
regressions); `Prism.Core.Tests` 151/153, and the 2 failures are exactly
`CiMiniGoldenTests.CiMini_Manifest_MatchesCommittedGolden` and `CiMini_MatchGolden_FamilyAssignmentsHold`
— the same pre-existing KNOWN-RED goldens already tracked by [[T-4980]] (Blocked, on [[T-5120]]) and the
Excel column-fill-rate gap ([[T-5130]]), not a new regression from this change.

**Direction 2 (scaled decode) — a tradeoff for the user, not decided here.** Skipping full-resolution
JPEG decode for the scale-invariant analyzers (composition, geometry, colour distribution, exposure)
would cut cost further, but it changes the actual measured pixel values feeding those analyzers, which
changes phenotype outputs on some images — the CiMini golden would need to be deliberately re-captured
and re-blessed, not just re-run. That's a real product decision (accept a one-time re-bless + validation
pass for the extra wall-clock savings, or leave direction 2 undone and keep today's full-resolution
values as the reference) that this review is not making. If wanted, it should be scoped as its own
ticket with an explicit re-bless step, not folded into a "yes/no" here.
...
>
> **Measured speedup, controlled (2026-08-11):** sequential vs `Parallel.ForEach` over the same 24
> SMASHEDLEMON45 images in one process, machine otherwise idle — **476 ms/image → 98 ms/image, 4.87×**
> (MaxDOP 8, 20 cores). So `YoloDetector.Detect`'s `RunLock` does *not* dominate Refine as feared: the
> decode and the geometry/colour analyzers around it parallelize freely and are the bulk of the cost.
>
> **A wall-clock trap worth recording.** The first end-to-end rerun after this change came back *slower*
> (43m02s vs the 36m17s baseline) and briefly looked like a regression. It was not — that job was
> sharing the machine with ~10 min of test suites, builds and commits, while the baseline ran nearly
> clean. **Two uncontrolled full-pipeline runs cannot measure a change of this size**; the isolated
> benchmark above is the real evidence. Match output was byte-identical across both runs (`ok=1732`),
> which is what the e2e run is actually good for.
>
...
**Closing note, 2026-08-11 — three findings above were real but not yet actioned; split into their own
tickets on close rather than left in the archive:** the chunk-barrier limiting pass 1 to ~3 of 8 threads
→ [[T-6930]]; the match-only run still paying for feature-analysis it never reads → [[T-6940]]; direction
2 (scaled decode) → [[T-6950]]. Only direction 1 (parallelize `RefinePhenotypes`) is what this ticket
closes as Done.
