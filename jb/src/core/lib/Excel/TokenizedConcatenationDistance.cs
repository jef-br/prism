...
/// <summary> Computes Tokenized Concatenation Distance for Excel header matching. </summary>
public static class TokenizedConcatenationDistance {

    // Maximum allowed number of tokens for which to search all permutations. Beyond this, the combinatorial explosion is too expensive.
    private const int MaxTokensForPermutationSearch = 16;


    /// <summary> Computes the tokenized concatenation distance between candidate tokens and a canonical target. </summary>
    ...
    public static double Compute(IReadOnlyList<string> tokens, string target, double reorderingWeight = 1.0) {
        ...
        if (tokens.Count == 0 || string.IsNullOrWhiteSpace(target)) return double.PositiveInfinity;

        string normalizedTarget = target.Trim();
        string[] normalizedTokens = tokens.Where(token => !string.IsNullOrWhiteSpace(token)).Select(token => token.Trim()).ToArray();

        // 4 early exit conditions: empty list, single token mismatch, token count mismatch, cheap combinatorial explosion (>2^N) guard.
        if (normalizedTokens.Length == 0) return double.PositiveInfinity;
        if (normalizedTokens.Length == 1) return normalizedTokens[0] == normalizedTarget ? 0.0 : double.PositiveInfinity;
        if (normalizedTokens.Sum(token => token.Length) != normalizedTarget.Length) return double.PositiveInfinity;
        if (normalizedTokens.Length > MaxTokensForPermutationSearch) return double.PositiveInfinity; //JB: Cheap block on O(2^N*N)

        int minimumKendallTau = ComputeMinimumKendallTau(normalizedTokens, normalizedTarget);
        if (minimumKendallTau < 0) return double.PositiveInfinity;
    ...
    /// <summary>Converts a TCD value into a percentage confidence.</summary>
    ...
    public static double ConvertDistanceToConfidence(double tcd, double lambda = 0.5) {
        ...
        // 100.0 is the percentage scale itself (per the method's own contract: 0-100), not a tunable value.
...
#pragma warning restore S109
    ...
    /// <summary> Minimum Kendall tau (inversion count) over all token permutations whose concatenation equals
    /// the target, or -1 when no permutation matches. Dynamic program over token-usage masks: a mask's position
    /// in the target is the total length of its used tokens, and appending token <c>i</c> adds one inversion
    /// for every already-used token with a higher original index at O(2^N × N). </summary>
    private static int ComputeMinimumKendallTau(string[] tokens, string target) {
        ...
        for (int mask = 0; mask < fullMask; mask++) {
            ...
            if (bestInversions[mask] == int.MaxValue) continue;
            ...
            for (int index = 0; index < tokenCount; index++) {
                ...
                if (position + token.Length > target.Length || !target.AsSpan(position, token.Length).SequenceEqual(token)) continue;
                ...
                if (candidate < bestInversions[nextMask]) bestInversions[nextMask] = candidate;
            }
        }
