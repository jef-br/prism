...
public sealed class ExcelFileHandler {
    ...
    private static ExcelWorkbook LoadOpenXmlWorkbook(string filePath) {
        ...
        IEnumerable<XElement> sheetElements = workbookDocument.Root?.Element(SpreadsheetNamespace + "sheets")?.Elements(SpreadsheetNamespace + "sheet") ?? [];
    ...
    private static string ReadOpenXmlCellValue(XElement cellElement, IReadOnlyList<string> sharedStrings) {
        ...
        if (cellType.Equals("inlineStr", StringComparison.OrdinalIgnoreCase)) {
            return cellElement.Descendants(SpreadsheetNamespace + "t").FirstOrDefault()?.Value.Trim() ?? string.Empty;
        }
    ...
    private static void ApplyVerticalMergedCells(XDocument worksheetDocument, Dictionary<int, Dictionary<int, string>> rowValues) {
        IEnumerable<XElement> mergeElements = worksheetDocument.Descendants(SpreadsheetNamespace + "mergeCells").Elements(SpreadsheetNamespace + "mergeCell");
        ...
        foreach (XElement mergeElement in mergeElements) {
            ...
            if (range is null || range.StartColumn != range.EndColumn) continue;
            if (!TryGetCellValue(rowValues, range.StartRow, range.StartColumn, out string? mergedValue) || string.IsNullOrWhiteSpace(mergedValue)) continue;
    ...
    private static bool TryGetCellValue(Dictionary<int, Dictionary<int, string>> rowValues, int rowIndex, int columnIndex, out string? cellValue) {
        cellValue = null;

        if (!rowValues.TryGetValue(rowIndex, out Dictionary<int, string>? row)) return false;
    ...
    private static ExcelWorksheet BuildWorksheet(string sourceFile, string worksheetName, Dictionary<int, Dictionary<int, string>> rowValues) {
        ...
        for (int rowIndex = 0; rowIndex <= lastRowIndex; rowIndex++) {
            Dictionary<int, string> row = rowValues.TryGetValue(rowIndex, out Dictionary<int, string>? existingRow) ? existingRow : [];
    ...
    private static int ReadOneBasedIndex(string? value) {
        return int.TryParse(value, NumberStyles.Integer, CultureInfo.InvariantCulture, out int parsedValue) ? parsedValue : 0;
    }
    ...
    private static int ReadColumnIndex(string cellReference) {
        ...
        if (string.IsNullOrWhiteSpace(columnLetters)) return -1;

        int columnNumber = 0;
        int lettersInTheAlphabet = 26;
        foreach (char letter in columnLetters.ToUpperInvariant()) {
            columnNumber *= lettersInTheAlphabet;
    ...
    private sealed record ExcelRange(int StartRow, int EndRow, int StartColumn, int EndColumn) {
        public static ExcelRange? TryParse(string reference) {
            ...
            if (startColumn < 0 || endColumn < 0 || startRow < 0 || endRow < startRow) return null;
