...
/// </summary>
public static class Analyzer_Interior {
    private const int MaxPatchSizeDivisor = 2;
    private const int BoundaryEdgeSideCount = 2;
    ...
    private static bool HasInteriorRegion(float[,] gray, float[,] edges, int w, int h, Config cfg) {
        float totalArea = w * h;
        ...
        for (int y0 = borderRing; y0 < h - borderRing; y0 += stepY) {
            for (int x0 = borderRing; x0 < w - borderRing; x0 += stepX) {
                for (int size = shortSide / 6; size <= shortSide / MaxPatchSizeDivisor; size += sizeStep) {
                    int y1 = y0 + size;
                    int x1 = x0 + size;
    ...
    private static float ComputeBoundaryEdge(float[,] edges, int x0, int y0, int x1, int y1) {
        float sum = 0f;
        int count = 0;
        for (int x = x0; x < x1; x++) { sum += edges[y0, x] + edges[y1 - 1, x]; count += BoundaryEdgeSideCount; }
        for (int y = y0 + 1; y < y1 - 1; y++) { sum += edges[y, x0] + edges[y, x1 - 1]; count += BoundaryEdgeSideCount; }
