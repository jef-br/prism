...
- pose-type (standing/sitting/crouching/lying): box aspect ratio first cut; yolo26s-pose ONNX if too coarse.

## To decide
- [ ] Haar cascades vs yolo26s-pose from the start (pose model gives keypoints, so head/body/pose come in one pass).
