...
- [ ] ONNX `InferenceSession` is created per pipeline run, not application-scoped
  ...
  - Answer (proposed pointer from existing data — `PRISM-classify.md` "ONNX Ownership" line "Sessions application-scoped … Per-batch buffers and per-worker state isolated behind `ImageClassifier.cs`"; M5 gate in `AGENTFEEDBACK.md` reads "ONNX session migrated to application-scoped singleton"; pending approval):
    Existing accepted docs already pick the option that satisfies your constraint. With a >1-minute typical job and a ≤5 s acceptable load delay, the per-job session's ~100–500 ms (one-time) load is negligible per job but its cost is paid by *every* concurrent job and holds N copies of the model in memory — the opposite of "least simultaneously-used server resources". An application-scoped singleton loads once, caps model memory at one copy regardless of concurrency, and keeps per-job overhead near zero. So: move the session to an application-scoped singleton owned by `ImageClassifier.cs` (per `PRISM-classify.md` ONNX Ownership) with thread-safe access; keep per-batch buffers and per-worker state isolated per the same doc so the shared session stays read-only across concurrent jobs.
    NOTE (verified against code 26/06/26 — NOT yet implemented): this todo's file pointer is stale. The session is no longer created in a `ShellStage_Classify`; it is now created per matching run at `MatchingService.cs:34` as `using IClassificationService classification = ClassificationService.Create(configuration)`, which constructs `new ImageClassifier()` in `ClassificationService.Create` (`ClassificationService.cs:29`) and disposes it when matching completes. The lifecycle is therefore still **per-job**, so the singleton migration remains open and the M5 gate ("ONNX session migrated to application-scoped singleton") is unmet.
    Implementation: hoist the `ImageClassifier`/session out of the per-run `using` in `MatchingService.Run` into an application-scoped owner, hand `ClassificationService` a shared reference instead of constructing one per call, and remove the per-run `Dispose()` of the classifier (dispose at app shutdown). No new data — this only realigns the stub with the already-accepted ONNX Ownership decision and the M5 gate wording.
