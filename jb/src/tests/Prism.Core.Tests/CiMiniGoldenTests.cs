...
public class CiMiniGoldenTests : IClassFixture<PipelineFixture> {
    ...
    public void CiMini_Manifest_MatchesCommittedGolden() {
        ...
        Assert.True(issues.Count == 0,
            ...
            + "KNOWN-RED (2026-08-06 merge, PipelineFixture widening): 2 families — 99985047 "
            + "(26182-Denim-801/a (1).jpg vs 87186790_2.jpg) and 99147525 (3 C153KB460011_*.png files). "
            + "CLIP produces different classification output for the same images depending on submission "
            + "transport (PipelineFixture's direct in-process call vs Invoke-CiPipeline.ps1's HTTP "
            + "multipart+zip-repack), close enough to the (now 0.42, was 0.33) orientation threshold to flip "
            + "which image wins a tied slot. [[T-2840]] confirmed the underlying batch-composition "
            + "sensitivity (max 0.045 delta, 0.069 signed range) and raised the threshold accordingly — "
            + "these 2 families are the residual risk that calibration alone may not fully close; re-check "
            + "after [[T-2840]]'s remaining steps land before assuming they need a different fix.\n"
            + "(98636303/OMB-E180-BV_1..6 — formerly listed here — was NOT a CLIP transport issue: root "
            + "cause was [[T-5100]], a Bracket-3/SiblingPropagator matching defect unrelated to "
            + "classification. Fixed 2026-08-11; no longer expected in this mismatch list.)\n\n"
