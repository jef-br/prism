...
**Missing model asset = loud startup failure (2026-07-20, user decision).** No model degrades silently to a lesser algorithm. `PrismConfiguration.ValidateModelAssets` fails config load without the YOLO or Real-ESRGAN asset, and `UpscaleService.Create` throws `PrismConfigurationException` when the model file is present but unloadable. Caveat (T-4110 review): the present-but-unloadable check exists only for Upscale — YOLO's and CLIP's `Initialize` still swallow a session-load failure into `IsReady = false` (file *existence* is loud, file *corruption* is not). Extending the unloadable-model check to YOLO/CLIP is open follow-up work, not covered by T-4110. The former monolith degrade path (T-2800's swallow in `PipelineServiceFactory.EnsureUpscalerReady`) is removed. The ×1.42 upscale KO bound is unrelated to any of this — it lives in `ImagePreProcessor` (`PREPROCESS_UPSCALE_EXCEEDED`, config `Output.Images.Resize.MAXIMUM_UpScale`) and is about refusing quality-destroying scale factors, not about hardware.
