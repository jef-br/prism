...
| ~~`clipping-path`~~ | **Removed 2026-08-04 ([[T-5030]], user decision).** It only ever meant "this file has an alpha channel", read straight off the decoded format. Import composites every input onto white and emits JPG before any analyzer runs, so downstream of Import the answer was structurally always `false` — the feature had never once been `true` in production, and its one use (an extra `anyOf` branch on the three `ghost-*` rules) was therefore dead. Deleted rather than redefined: nothing measurable on a flattened JPG distinguishes "cut out against a flat background" from "shot on a seamless white sweep". | — | — | — | — |
| `transparent-background` | Whether the image background is fully transparent. **Structural since T-5030**: Import flattens all alpha, so this is now an unconditional `false`, not a per-image measurement. Retained as diagnostic surface; no phenotype rule consumes it | Boolean | `true`, `false` | low | very-high |
...
| `interior-detected` | Whether the image looks down into an enclosed cavity (a bag compartment, a shoe, a box). Sobel gradient + patch-grid scan: interior smoother than its surrounding ring, bounded by strong edges, well inside the frame. Producer `Analyzer_Interior`, thresholds in `analyzer_Config.json` → `Interior`. Consumed by the `interior-shot` phenotype. **Added to this catalog 2026-08-04 (T-5040) — the feature and its producer already existed and were declared in `ImageNGP.json`; only this row was missing** | Boolean | `true`, `false` | medium | medium |
| `is-illustration` | Whether the image is a drawing/rendering rather than a photograph (flat regions, limited palette, hard synthetic edges). Producer `Analyzer_IsIllustration`. Consumed by the `illustration-technical-drawing` phenotype. **Added to this catalog 2026-08-04 (T-5040), same reason as above** | Boolean | `true`, `false` | medium | medium |
