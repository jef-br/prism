...
- Use `AGENTS.md`, code, configs, comments, `jb\docs` and nearby `.md` files as supporting repo knowledge.
- Do not edit `AGENTS.md` unless the user explicitly grants permission for that specific edit.

## Creating Todos

- Create each todo in the nearest folder-local `jbtodo.md`.
- Stay close to the user's original phrasing and scope.
- Do not expand into adjacent feature work unless the repo context proves that extra scope is required for a valid decision.

Before adding a new todo:
  1. Check in `jb\docs` to see the todo would not contradict documentation/project intent (start with `docs/prism/PRISM-index.md`)
  2. check existing todos to avoid contradictions and duplicates.
  3. Ask questions until the todo phrasing is clear enough to remove unintended ambiguity.
