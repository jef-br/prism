...
**Answer:** OPEN — `core/Images/*` path sweep done 2026-07-17, broader vocabulary sweep still to do
...
`Services/Upscale/Engine/...`), `Prism.Core.Images.*` assembly names (resolved, T-3700).

**`core/Images/*` path drift — CLOSED 2026-07-17.** All 25 confirmed instances across the 8 inventoried
doc files fixed, each verified against the file's actual current location (not a blind `Images/`→`Services/`
substitution — several targets moved further than that, e.g. `BoundingBox.cs` is now under `core/Models/`,
not under any `Services/Transform/` path, and config JSON like `MatchingConfig.json`/`TranslationDictionary.json`
centralized to `core/config/` rather than living under `Services/Matching/...`):
- `GLOSSARY.md` (2): MCFG, DOR → both now point at `jb/src/core/config/`.
- `PRISM-transform-generate.md` (7): `ImagePreProcessor.cs` → `Services/Matching/`;
  `Tx_LowContrastEnhancement.cs` → `Services/Transform/Engine/Utils/`; Real-ESRGAN onnx →
  `Services/Upscale/Engine/ONNX/`; Transform rules folder → `Services/Transform/Engine`; Transform
  `jbtodo.md` → `Services/Transform/Engine/jbtodo.md`; `BoundingBox.cs` → `core/Models/`; Generate
  `jbtodo.md` → `Services/Generate/jbtodo.md`.
- `PRISM-classify.md` (4): Classify `jbtodo.md`, ONNX model path → `Services/Matching/Classify/...`;
  `ClipPrompts.json` → `core/config/`; `Analyzer_IsIllustration.cs` → `Services/Matching/Analyzers/`
  (not `Classify/Analyzers/` — Analyzers is its own sibling folder).
- `PRISM-match.md` (4): `ImageMatcher.cs` → `Services/Matching/`; CLIP ONNX dir →
  `Services/Matching/Classify/ONNX/...`; Translate code → `Services/Matching/Match/Translate/`;
  `TranslationDictionary.json` → `core/config/`.
- `meta/currentMatching.md` (4): Match folder + `jbtodo.md` → `Services/Matching/Match/...`;
  `MatchingConfig.json`/`TranslationDictionary.json` → `core/config/`.
- `meta/MatchingDeltaVsTarget.md` (2): both Match `jbtodo.md` refs → `Services/Matching/Match/jbtodo.md`.
- `ImageNGP/ImageFeatures.md` (1): `BoundingBox.cs` → `core/Models/`.
- `ideas-on-NGP.md` (1): Order stage → `Services/Matching/Order/`.

**Still open, found but out of the confirmed-25 scope:** `PRISM-information.md:35` (repo root, not under
`jb/docs/`) still has `jb/src/core/Images/Classify/ONNX/clip-vit-b32-uint8/model_uint8.onnx` — same fix as
`PRISM-match.md`/`PRISM-classify.md` above (→ `Services/Matching/Classify/ONNX/...`), just never inventoried
because the original grep only covered `jb/docs/`.

**Still open, not started:** the broader vocabulary pass this todo also asks for — stale type/field/attribute/
config-key names beyond file paths, the PRISM-specific-vocabulary check (FamilyID, `_det#`, ImageNGP,
ImageRole, DetOrderRules, IEM, KO, Batch, phenotype, LAMBDA/INPUT/OUTPUT), and the GLOSSARY.md
retire/add pass. None of that was in scope for this pass and none of it was checked.
