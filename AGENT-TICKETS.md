...
**Scope extension (2026-07-20, user):** no algorithm switching on GPU presence — Upscale now loads Real-ESRGAN on every host (CPU EP when no adapter, like CLIP/YOLO); Lanczos4 (`Upscaler_c_p_u`) is only the missing-model fallback (T-2800 degrade path), never a CPU-mode substitute. `UpscaleService.Create` gate removed; decision recorded in `PRISM-model-runtime.md`.
