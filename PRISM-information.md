...
### jb/src/workbench/wpf/jbtodo.md
...
- Define `Prism.Process` output type: name the C# type that returns output images, manifest data, KO details, and processing diagnostics.
  - Answer:
    ...
    - `Guid JobID`: PRISM's internal job ID generated by `Prism.Process`.
...
## Completed Todo Decisions Synced From Local jbtodo.md Files

Initial completed decisions were moved out of folder-local `jbtodo.md` files on 2026-05-19. Folder-local `jbtodo.md` files now keep open questions only; completed decisions belong in this project information file.

### jb/src/api/jbtodo.md
...
- Define API progress streaming behavior: choose how clients receive stage progress while a long batch is processing.
  - Answer:
    ...
    Submission behavior:
    - `POST /PRISM/process` accepts the configured multipart request and starts a PRISM job.
    ...
    - PRISM keeps its own internally generated `JobID`; caller-provided tokens remain correlation tokens only.
...
### jb/src/core/Images/Classify/jbtodo.md
...
### jb/src/core/Images/Classify/ONNX/clip-vit-b32-uint8/jbtodo.md
...
### jb/src/core/Images/Classify/ONNX/jbtodo.md
...
- Define ONNX runtime provider policy: say whether CPU is always supported and when GPU providers may be used.
  - Answer:
    ...
    GPU runtime providers may be used only when they are explicitly configured and available. Missing, invalid, or incompatible required model files remain PRISM-owned failures that fail fast and loud; GPU absence alone is not such a failure.

- Define ONNX fallback behavior without GPU: say how Prism behaves on local servers and laptops with CPU only.
  - Answer:
    Prism must run on local servers whose hardware may not include a GPU.
...
### jb/src/core/Images/Match/jbtodo.md
...
- Define numeric length-penalty scoring: specify how shorter or longer numeric candidates are penalized.
  - Answer:
    ...
    If the candidate token set is otherwise an identical match but the column token is longer than the filename token, subtract `1 - (length difference / total column length)` from the score. Example: `abcde` compared to `abcdefgh` has a length difference of `3` over total length `8`, producing `1 - 5/8 = -0.375` as documented in `PRISM-information.md`.
...
### jb/src/core/Images/Transform/jbtodo.md
...
- Define source tree ownership rules: state which top-level folder owns API notes, core pipeline notes, workbench notes, shared docs, and test fixtures.
  - Answer:
    ...
    - `PRISM-information.md` owns established project knowledge; folder-local `jbtodo.md` files own unresolved decisions.

### jb/src/workbench/jbtodo.md
...
### jb/src/workbench/web/jbtodo.md
